...
def test_public_facade_rejects_unsupported_parameter_locations_and_recovers():
...
def test_public_facade_rejects_a_schema_less_parameter():
    invalid = deepcopy(PARAMETER_MEDIA_SCHEMA)
...
def test_public_facade_rejects_duplicate_parameter_identities():
    invalid = deepcopy(PARAMETER_MEDIA_SCHEMA)
    invalid["paths"]["/records"]["get"]["parameters"] = [
...
def test_public_facade_prefers_json_request_body_fields():
...
def test_public_facade_rejects_non_json_request_body_media(content):
    invalid = deepcopy(PARAMETER_MEDIA_SCHEMA)
