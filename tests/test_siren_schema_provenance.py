...
class TestSirenSchemaProvenance:
    def test_vendored_schema_matches_the_pinned_source_and_digest(self):
        schema = files(modwire_siren).joinpath("siren_schema/schema/siren.schema.json")
        provenance = files(modwire_siren).joinpath("siren_schema/schema/siren.schema.provenance.json")
