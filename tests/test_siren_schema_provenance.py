...
class TestSirenSchemaProvenance:
    def test_vendored_schema_matches_the_pinned_source_and_digest(self):
        schema = files(modwire_siren).joinpath("shared/siren_schema/values/siren.schema.json")
        provenance = files(modwire_siren).joinpath("shared/siren_schema/values/siren.schema.provenance.json")
